...
def template_sencillo(esperado, nombre, tarea, argumentos=[]):
    ...
    def decoradora(funcion):
        def contenedora(f, uuid):
            error = False
            resultados = []
            excepcion = None
            try:
                resultado_obtenido = funcion(f, uuid)(*argumentos)
            ...
            helper(resultados, tarea, nombre, uuid, error=error, excepcion=excepcion, exception_type=type(e))
        return contenedora
    return decoradora
...
def template_iterable(lista, nombre, tarea):
    ...
    def decoradora(funcion):
        def contenedora(f, uuid):
            ...
            for tupla in lista:
                try:
                    resultado_obtenido = funcion(f, uuid)(*tupla[0])
