    ...
    def decoradora(funcion):
        def contenedora(f, uuid, deseo_ayudar):
            ...
            try:
                ...
                resultados.append(Resultado(None, tuple(map(tuple,resultado_esperado)), tuple(map(tuple,resultado_obtenido)), estado.all()).__dict__)
        ...
        def contenedora(f, uuid, deseo_ayudar):
            ...
            for tupla in lista:
                try:
                    ...
                    resultados.append(Resultado(tupla[0], resultado_esperado.tolist(), resultado_obtenido.tolist(), bool(estado)).__dict__)
                except Exception as e:
                    mensaje_error(e)
                    error = True
                    excepcion = str(e)
