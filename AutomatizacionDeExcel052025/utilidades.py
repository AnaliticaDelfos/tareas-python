...
import pandas as pd
errores = []
ID_CURSO = "ehE6T1Au2mX1uTqQUaUW"
correo = os.environ.get("JUPYTERHUB_USER")

def __convert_from_type_to_dict(resultados):
    for i in range(len(resultados)):
        if type(resultados[i]['esperado']) == pd.Series or type(resultados[i]['esperado']) == pd.DataFrame:
            resultados[i]['esperado'] = resultados[i]['esperado'].to_dict()
        if type(resultados[i]['obtenido']) == pd.Series or type(resultados[i]['obtenido']) == pd.DataFrame:
            resultados[i]['obtenido'] = resultados[i]['obtenido'].to_dict()
        if resultados[i]['argumentos']:
            for argumento in resultados[i]['argumentos']:
                if type(argumento) == pd.Series or type(argumento) == pd.DataFrame:
                    resultados[i]['argumentos'][i] = argumento.to_dict()

def mandar_a_firestore(ejercicio, calificacion, resultados, tarea):
    __convert_from_type_to_dict(resultados)
